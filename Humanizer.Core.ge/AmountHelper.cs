...
namespace Humanizer.Core.ge
{
    public class AmountHelper
    {
        ...
        public static string AmountToWords(decimal amount, string currencyName, string coinName)
        {
            return AmountToWords(Convert.ToDouble(amount), currencyName, coinName);
        }

        public static string AmountToWords(int amount, string currencyName, string coinName)
        {
            return AmountToWords(Convert.ToDouble(amount), currencyName, coinName);
        }

        public static string AmountToWords(double amount, string currencyName, string coinName)
        {
            if (Math.Abs(amount - 0) < 0.001)
                return "ნული " + currencyName;
            string output = "";
            ...
            output += LongToGeoString(dm) + string.Format(" {0}", currencyName);
            if (dc != 0) output += " და " + LongToGeoString(Convert.ToInt64(dc)) + string.Format(" {0}", coinName);
            return output.Trim();
        }
